"""TradingAgents-波波个股分析 — Streamlit Web UI."""
...
st.set_page_config(
    page_title="TradingAgents-波波个股分析",
    page_icon="📈",
    layout="wide",
...
def _require_login() -> None:
    password = _read_secret("APP_PASSWORD")
    if not password or st.session_state.get("auth_ok"):
        return

    username = _read_secret("APP_USERNAME")
    st.markdown("### TradingAgents-波波个股分析")
    with st.form("login_form"):
        entered_username = ""
        if username:
            entered_username = st.text_input("用户名")
        entered_password = st.text_input("访问密码", type="password")
        submitted = st.form_submit_button("登录", type="primary")

    if submitted:
        username_ok = not username or hmac.compare_digest(entered_username, username)
        password_ok = hmac.compare_digest(entered_password, password)
...
# State 0: Idle — welcome screen
    ...
    st.markdown(
        ...
        ">
            ...
            ">
                <span style="color: #ff5a1f;">TradingAgents</span><span style="color: #f5f1eb;">-</span><span style="color: #ff5a1f;">波波个股分析</span>
            </div>
