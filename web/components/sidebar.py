...
def _render_llm_config() -> None:
    ...
    if provider_key in MODEL_OPTIONS:
        ...
        quick_label_by_value = {value: label for label, value in quick_options}
        quick_values = [value for _, value in quick_options]
        deep_label_by_value = {value: label for label, value in deep_options}
        deep_values = [value for _, value in deep_options]

        quick_model = st.selectbox(
            "快速思考模型",
            quick_values,
            format_func=lambda value: quick_label_by_value[value],
            key=f"{provider_key}_quick_model",
            help="用于常规分析任务，速度优先",
        )
        st.session_state["quick_think_llm"] = quick_model
        if st.session_state["quick_think_llm"] == "custom":
            st.session_state["quick_think_llm"] = st.text_input(
                "Custom quick model ID",
                key=f"{provider_key}_custom_quick_model",
        ...
        deep_model = st.selectbox(
            "深度思考模型",
            deep_values,
            format_func=lambda value: deep_label_by_value[value],
            key=f"{provider_key}_deep_model",
            help="用于辩论/决策等需要深度推理的任务",
        )
        st.session_state["deep_think_llm"] = deep_model
        if st.session_state["deep_think_llm"] == "custom":
            st.session_state["deep_think_llm"] = st.text_input(
                "Custom deep model ID",
                key=f"{provider_key}_custom_deep_model",
                placeholder="e.g. gpt-5.5",
            ).strip()
    else:
...
def render_sidebar() -> None:
    ...
    st.markdown(
        ...
        <div style="text-align:center; margin-bottom:1.5rem;">
            <span style="font-size:2rem; font-weight:800; color:#ff5a1f;">TradingAgents</span><span style="font-size:2rem; font-weight:800; color:#f5f1eb;">-</span><span style="font-size:2rem; font-weight:800; color:#ff5a1f;">波波个股分析</span>
            <div style="font-size:0.85rem; color:#888; margin-top:0.2rem;">
                A股多Agent个股分析系统
            </div>
            <div style="font-size:0.7rem; color:#555; margin-top:0.3rem;">
                by <a href="https://github.com/Fantasy-Bo/Trading" style="color:#ff5a1f; text-decoration:none;">Fantasy-Bo</a>
            </div>
        </div>
        """,
